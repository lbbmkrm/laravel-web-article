<x-layout>
  ...
  <x-slot:tittle>{{ $web['title'] }}</x-slot>
  <x-slot:content>{{ $web['content'] }}</x-slot>
</x-layout>

<form class="max-w-md mx-auto lg:mt-8"  method="GET">   
    ...
    @foreach ($posts as $post)
      <article class="p-6 bg-white rounded-lg border border-gray-200 shadow-md hover:scale-105 transition-all duration-700 dark:bg-gray-800 dark:border-gray-700">
        ...
        <div class="flex justify-between items-center">
          ...
          <a href="{{ route('blog.full' , $post->slug) }}" class="inline-flex items-center font-medium text-primary-600 dark:text-primary-500 hover:underline">
            Read more
